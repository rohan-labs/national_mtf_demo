...
export async function POST(request: NextRequest) {
  try {
    ...
    // Prepare the input array with conversation history
    const input = [
      ...conversationHistory,
    ...
    const response = await openai.responses.create({
      model: 'gpt-4.1-mini',
      instructions: `You are a Q&A bot for the National MedTech Foundation (MTF). Answer questions for users regarding the National MTF only using the information provided. If they ask a question that is not related to the National MTF, respond with: "Sorry, I can't help with that but feel free to reach out to the National MTF team!"

Key information about the National MTF:
...
Always be helpful and informative about MTF activities, events, team members, and opportunities.`,
      input,
      store: false, // Set to true if you want to store responses for 30 days
    });
